...
class ImageProcessing {
 public:
  ...
  template <typename T>
  static cv::Mat GenerateImageByData(const std::vector<T>& image_data,
                                     const int image_width,
                                     const int image_height);

  static bool IsPixelPositionInImage(const Eigen::Vector2f& pixel_position,
                                     const int image_width,
                                     const int image_height);
  ...
  static float CalculateSumOfSquaredDistance(
      const std::vector<float>& intensity_list_1,
      const std::vector<float>& intensity_list_2);
  static float CalculateSumOfSquaredDistance(const cv::Mat& cv_image_1,
                                             const cv::Mat& cv_image_2);

  static cv::Mat DownsampleImage(const cv::Mat& cv_source_image);
  static cv::Mat GeneratePaddedImageByMirroring(const cv::Mat& cv_source_image,
                                                const int pad_size);
  static std::vector<cv::Mat> GenerateImagePyramid(
  ...
  // &pyramid,
  //                           const int max_level, const bool use_padding,
  //                           const int pad_size, std::vector<cv::Mat>
  //                           &pyramid_du, std::vector<cv::Mat> &pyramid_dv);

 public:
  static float InterpolateImageIntensity(const cv::Mat& cv_source_image,
                                         const Eigen::Vector2f& pixel_position);
  static std::vector<float> InterpolateImageIntensity(
      const cv::Mat& cv_source_image,
  ...
  static std::vector<float> InterpolateImageIntensityWithPatchSize(
      const cv::Mat& cv_source_image,
      const Eigen::Vector2f& patch_center_pixel_position, const int patch_width,
      const int patch_height);
  static std::vector<float> InterpolateImageIntensityWithIntegerRow(
      const cv::Mat& cv_source_image, const float patch_center_u,
      const int patch_center_v_floor, const int patch_width,
      const int patch_height);
  static std::vector<float> InterpolateImageIntensityWithIntegerColumn(
      const cv::Mat& cv_source_image, const int patch_center_u_floor,
      const float patch_center_v, const int patch_width,
      const int patch_height);

  static std::vector<float> InterpolateImageIntensity_unsafe(
      const cv::Mat& cv_source_image,
      const std::vector<Eigen::Vector2f>& pixel_position_list);
...
cv::Mat ImageProcessing::GenerateImageByData(const std::vector<T>& image_data,
  ...
  for (int i = 0; i < image_data.size(); ++i)
    values[i] = static_cast<uint8_t>(
        std::clamp(static_cast<int>(std::round(image_data[i])), 0, 255));

  cv::Mat cv_image_interp(image_height, image_width, CV_8UC1);
  memcpy(cv_image_interp.ptr<uint8_t>(0), values.data(),
         sizeof(uint8_t) * num_data);
