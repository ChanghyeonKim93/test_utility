...
bool ImageProcessing::IsPixelPositionInImage(
    const Eigen::Vector2f& pixel_position, const int image_width,
    const int image_height) {
  const auto& pixel_u = pixel_position.x();
  const auto& pixel_v = pixel_position.y();
  return (pixel_u > 0 && pixel_u <= image_width - 1 && pixel_v > 0 &&
          pixel_v <= image_height - 1);
}

bool ImageProcessing::IsPixelPositionInImage(const float pixel_u,
                                             const float pixel_v,
...
float ImageProcessing::CalculateSumOfSquaredDistance(
    const std::vector<float>& intensity_list_1,
    const std::vector<float>& intensity_list_2) {
  if (intensity_list_1.size() != intensity_list_2.size())
    throw std::runtime_error(
        "intensity_list_1.size() != intensity_list_2.size()");
...
float ImageProcessing::CalculateSumOfSquaredDistance(
    const cv::Mat& cv_image_1, const cv::Mat& cv_image_2) {
  float ssd = 0;
  if (cv_image_1.cols != cv_image_2.cols || cv_image_1.rows != cv_image_2.rows)
    throw std::runtime_error(
        "cv_image_1.cols != cv_image_2.cols || cv_image_1.rows != "
        "cv_image_2.rows");

  if (cv_image_1.type() != CV_8UC1 || cv_image_2.type() != CV_8UC1)
    throw std::runtime_error(
        "cv_image_1.type() != CV_8UC1 || cv_image_2.type() != CV_8UC1\n");
  ...
  for (; ptr1 != ptr1_end; ++ptr1, ++ptr2) {
    const float diff =
        static_cast<float>(*(ptr1)) - static_cast<float>(*(ptr2));
    ssd += diff * diff;
  }
  ssd /= static_cast<float>(num_elements);
...
cv::Mat ImageProcessing::DownsampleImage(const cv::Mat& cv_source_image) {
  ...
  cv::Mat cv_result_image =
      cv::Mat(half_image_height, half_image_width, CV_8UC1);
  ...
  const uint8_t* result_ptr_end =
      result_ptr + half_image_width * half_image_height;
  while (result_ptr != result_ptr_end) {
    const uint8_t* source_image_row_end_ptr = source_I00_ptr + image_width;
    for (; source_I00_ptr != source_image_row_end_ptr;
         ++result_ptr, source_I00_ptr += 2, source_I01_ptr += 2,
         source_I10_ptr += 2, source_I11_ptr += 2) {
      const auto& I00 = *source_I00_ptr;
      const auto& I01 = *source_I01_ptr;
      const auto& I10 = *source_I10_ptr;
      const auto& I11 = *source_I11_ptr;
      *result_ptr = static_cast<uint8_t>(
          static_cast<uint16_t>(I00 + I01 + I10 + I11) >> 2);
    }
...
cv::Mat ImageProcessing::GeneratePaddedImageByMirroring(
    const cv::Mat& cv_source_image, const int pad_size) {
  const int image_width = cv_source_image.cols;
  const int image_height = cv_source_image.rows;
  const int image_width_padded = image_width + 2 * pad_size;
  const int image_height_padded = image_height + 2 * pad_size;

  cv::Mat cv_result_image = cv::Mat::zeros(
      image_height_padded, image_width_padded, cv_source_image.type());

  const uint8_t* source_ptr = cv_source_image.ptr<uint8_t>(0);
  const uint8_t* source_ptr_end = source_ptr + image_width * image_height;
  uint8_t* result_ptr = cv_result_image.ptr<uint8_t>(0) +
                        pad_size * image_width_padded + pad_size;
  ...
  uint8_t* ptr_dst_row_end =
      cv_result_image.ptr<uint8_t>(image_height_padded - pad_size);
  ...
  source_ptr_end =
      cv_result_image.ptr<uint8_t>(0) + 2 * pad_size * image_width_padded;
  result_ptr =
      cv_result_image.ptr<uint8_t>(0) + (pad_size - 1) * image_width_padded;
  for (; source_ptr != source_ptr_end;
  ...
  // lower pad
  source_ptr = cv_result_image.ptr<uint8_t>(0) +
               (image_height_padded - 2 * pad_size) * image_width_padded;
  source_ptr_end = cv_result_image.ptr<uint8_t>(0) +
                   (image_height_padded - pad_size) * image_width_padded;
  result_ptr = cv_result_image.ptr<uint8_t>(0) +
               (image_height_padded - 1) * image_width_padded;
  for (; source_ptr != source_ptr_end;
       source_ptr += image_width_padded, result_ptr -= image_width_padded) {
    memcpy(result_ptr, source_ptr, size_of_padded_row);
  }
...
std::vector<cv::Mat> ImageProcessing::GenerateImagePyramid(
    const cv::Mat& cv_source_image, const int num_levels,
  ...
  for (int lvl = 1; lvl < num_levels; ++lvl) {
    image_pyramid[lvl] =
        ImageProcessing::DownsampleImage(image_pyramid[lvl - 1]);
  }
  ...
  if (use_padding) {
    ...
    for (int lvl = num_levels - 1; lvl >= 0; --lvl) {
      image_pyramid[lvl] = ImageProcessing::GeneratePaddedImageByMirroring(
          image_pyramid[lvl], pad_size);
    }
  }
...
// void GenerateImagePyramid(const cv::Mat &img_src, std::vector<cv::Mat>
// &pyramid,
//                           const int max_level, const bool use_padding, const
//                           int pad_size, std::vector<cv::Mat> &pyramid_du,
//                           std::vector<cv::Mat> &pyramid_dv) {
...
float ImageProcessing::InterpolateImageIntensity(
    const cv::Mat& cv_image, const Eigen::Vector2f& pixel_position) {
  ...
  if (!ImageProcessing::IsPixelPositionInImage(pixel_position, image_width,
                                               image_height))
    return -1.0f;
  ...
  interpolated_intensity = axay * (I00 - I01 - I10 + I11) + ax * (-I00 + I01) +
                           ay * (-I00 + I10) + I00;
...
std::vector<float> ImageProcessing::InterpolateImageIntensity(
  ...
  std::vector<float>::iterator itr_interpolated_intensity =
      interpolated_intensity_list.begin();
  int count = 0;
  std::vector<Eigen::Vector2f>::const_iterator itr_pixel =
      pixel_position_list.begin();
  for (; itr_interpolated_intensity != interpolated_intensity_list.end();
    ...
    if (!ImageProcessing::IsPixelPositionInImage(pixel, image_width,
                                                 image_height))
      continue;
    ...
    const uint8_t* source_I00_ptr =
        source_ptr + v_floor * image_width + u_floor;
    ...
    const float interpolated_intensity = axay * (I00 - I01 - I10 + I11) +
                                         ax * (-I00 + I01) + ay * (-I00 + I10) +
                                         I00;
...
std::vector<float> ImageProcessing::InterpolateImageIntensityWithPatchPattern(
  ...
  std::vector<float>::iterator itr_interpolated_intensity =
      interpolated_intensity_list.begin();
  std::vector<Eigen::Vector2i>::const_iterator itr_pixel =
      patch_local_pixel_position_list.begin();
  for (; itr_interpolated_intensity != interpolated_intensity_list.end();
       ++itr_interpolated_intensity, ++itr_pixel) {
    ...
    if (!ImageProcessing::IsPixelPositionInImage(u_floor, v_floor, image_width,
                                                 image_height))
      continue;
...
std::vector<float> ImageProcessing::InterpolateImageIntensityWithPatchSize(
  ...
  std::vector<float>::iterator itr_interpolated_intensity =
      interpolated_intensity_list.begin();
  for (int v = v_center_floor - half_patch_height;
       v <= v_center_floor + half_patch_height; ++v) {
    const uint8_t* source_row_ptr = source_ptr + v * image_width;
    for (int u = u_center_floor - half_patch_width;
         u <= u_center_floor + half_patch_width; ++u) {
      if (!ImageProcessing::IsPixelPositionInImage(u, v, image_width,
                                                   image_height))
        continue;
...
std::vector<float> ImageProcessing::InterpolateImageIntensityWithIntegerRow(
    const cv::Mat& cv_image, const float patch_center_u,
    const int patch_center_v_floor, const int patch_width,
...
//     if (pt.x < 0 || pt.x > image_width - 1 || pt.y < 0 || pt.y > image_height
//     - 1)
//       continue;
...
//     float I_interp = axay * (I1 - I2 - I3 + I4) + ax * (-I1 + I2) + ay * (-I1
//     + I3) + I1;
...
// void InterpolateImageIntensitySameRatioRegularPattern(const cv::Mat
// &cv_image,
//                                                       const cv::Point2f
//                                                       &pt_center, int
//                                                       win_size,
...
//   const cv::Point2i pt_center0(static_cast<int>(pt_center.x),
//   static_cast<int>(pt_center.y)); const float ax = pt_center.x -
//   pt_center0.x; const float ay = pt_center.y - pt_center0.y; const float axay
//   = ax * ay;
// }
...
std::vector<float>
ImageProcessing::InterpolateImageIntensityWithPatchPattern_unsafe(
    const cv::Mat& cv_image, const Eigen::Vector2f& patch_center_pixel_position,
    const std::vector<Eigen::Vector2i>& patch_local_pixel_position_list) {
  if (cv_image.type() != CV_8U)
    throw std::runtime_error("cv_image.type() != CV_8U");
...
std::vector<float>
ImageProcessing::InterpolateImageIntensityWithPatchSize_unsafe(
    const cv::Mat& cv_image, const Eigen::Vector2f& patch_center_pixel_position,
    const int patch_width, const int patch_height) {
  if (cv_image.type() != CV_8U)
    throw std::runtime_error("cv_image.type() != CV_8U");
  ...
  std::vector<float>::iterator itr_interpolated_intensity =
      interpolated_intensity_list.begin();
  const uint8_t* source_row_ptr =
      source_ptr + (v_center_floor - half_patch_height) * image_width;
  for (int v = v_center_floor - half_patch_height;
       v <= v_center_floor + half_patch_height; ++v) {
    for (int u = u_center_floor - half_patch_width;
         u <= u_center_floor + half_patch_width; ++u) {
...
std::vector<float>
ImageProcessing::InterpolateImageIntensityWithIntegerRow_unsafe(
    const cv::Mat& cv_image, const float patch_center_u,
    const int patch_center_v_floor, const int patch_width,
    const int patch_height) {
  if (cv_image.type() != CV_8U)
...
std::vector<float>
ImageProcessing::InterpolateImageIntensityWithIntegerRow_simd_intel(
    const cv::Mat& cv_image, const float patch_center_u,
    const int patch_center_v_floor, const int patch_width,
    const int patch_height) {
  // static float *buf_u_ = nullptr, *buf_v_ = nullptr, *buf_I00_ = nullptr,
  // *buf_I01_ = nullptr,
  //              *buf_I10_ = nullptr, *buf_I11_ = nullptr, *buf_interped_ =
  //              nullptr;
  // if (buf_u_ == nullptr) {
