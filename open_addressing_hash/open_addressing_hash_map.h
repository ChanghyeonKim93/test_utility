...
class OpenAddressingHash {
 public:
  ...
  OpenAddressingHash(size_t size) : num_activated_slot_(0) {
    // 테이블 크기를 2의 제곱으로 설정
    num_slot_ = next_power_of_two(size);
  ...
  class iterator {
   public:
    ...
    iterator(OpenAddressingHash* hash, size_t index)
      ...
      // 생성 시 유효한 위치로 이동
      if (index_ < hash_->num_slot_ && (!hash_->slots_[index_].is_occupied ||
                                        hash_->slots_[index_].is_deleted)) {
        advance_to_valid();
      }
    }
    ...
    reference operator*() const {
      return {hash_->slots_[index_].key, hash_->slots_[index_].value};
    }

    // 화살표 연산자 추가
    pointer operator->() const {
      static value_type result = {hash_->slots_[index_].key,
                                  hash_->slots_[index_].value};
      return &result;
    }
   ...
   private:
    ...
    void advance_to_valid() {
      while (index_ < hash_->num_slot_ && (!hash_->slots_[index_].is_occupied ||
                                           hash_->slots_[index_].is_deleted)) {
        ++index_;
      }
    }

    OpenAddressingHash* hash_;
    size_t index_;
  ...
  class const_iterator {
   ...
   private:
    ...
    const OpenAddressingHash* hash_;
    size_t index_;
  };
 ...
 private:
  void Resize(const size_t new_size) {
    size_t new_num_slot = next_power_of_two(new_size);
