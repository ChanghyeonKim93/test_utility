...
while(1)
  ...
  if(~is_stopby_passed && 
    abs(Xk(1) - X_stopby(1)) < 0.5 && abs(Xk(2) - X_stopby(2)) < 0.1)
    %parameters.control.gain_lateral_error = 1.5;
    %parameters.control.gain_heading_error = 3.5;
    is_stopby_passed = true;
    X_des = X_goal;
  end
  if(is_stopby_passed && 
    abs(Xk(1) - X_goal(1)) < 0.02 && abs(Xk(2) - X_goal(2)) < 0.02)
    break;
  end

  direction_sign = -1;
  Uk = lateral_heading_error_controller2(Xk, U_delay(:,1), X_des, parameters, direction_sign);
  ...
  f = [
    U_sim(1)*cos(Xk(3)); 
    U_sim(1)*sin(Xk(3)); 
    U_sim(1)/parameters.steer_to_rotation_center*sin(U_sim(2))
  ];
...
figure('position',[10,10,540,1080]);
