  ...
  class RelativePoseCostFunctor {
   public:
    RelativePoseCostFunctor(const Pose& relative_pose_01)
        : rel_pos_01_(relative_pose_01.translation()),
          rel_quat_01_(utility::GeometryHelper::ConvertToQuaternion(
              Eigen::Matrix3d(relative_pose_01.linear()))) {}
    ...
    bool operator()(const T* const pos0, const T* const rvec0,
      ...
      Eigen::Quaternion<T> error_q =
          rel_quat_01_.template cast<T>().conjugate() * (q0.conjugate() * q1);

      Eigen::Map<Eigen::Matrix<T, 6, 1>> residual(residual_ptr);
      residual.template block<3, 1>(0, 0) =
          q0 * rel_pos_01_.template cast<T>() - (t1 - t0);
