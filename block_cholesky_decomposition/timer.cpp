...
double toc(bool flag_verbose) {
  ...
  if (flag_verbose) {
    std::cout << " exec time: "
              << (double)(gap / std::chrono::microseconds(1)) / 1000.0
              << "[ms]\n";
  }
...
StopWatch::StopWatch(const std::string& timer_name) : timer_name_(timer_name) {}
...
double StopWatch::start(bool flag_verbose) {
  ...
  if (flag_verbose) {
    std::cout << "[" << timer_name_ << "]    start at: " << gap_in_msec
              << " [ms]\n";
  }
...
double StopWatch::lapTimeFromStart(bool flag_verbose) {
  ...
  double gap_in_msec = (double)(gap / std::chrono::microseconds(1)) * 0.001;
  if (flag_verbose) {
    std::cout << "[" << timer_name_ << "] lap time at: " << gap_in_msec
              << " [ms]\n";
  }
...
double StopWatch::stop(bool flag_verbose) {
  ...
  double gap_in_msec = (double)(gap / std::chrono::microseconds(1)) * 0.001;
  if (flag_verbose) {
    std::cout << "[" << timer_name_ << "]      end at: " << gap_in_msec
              << " [ms]\n";
  }
