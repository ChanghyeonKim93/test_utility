...
Numeric* cs_chol(const SparseMatrix* A, const Symbolic* S) {
  ...
  n = A->n;
  N = reinterpret_cast<Numeric*>(cs_calloc(1, sizeof(Numeric)));
...
Symbolic* cs_schol(const SparseMatrix* A, int order) {
  ...
  if (!A) return (NULL); /* check inputs */
  n = A->n;
  S = reinterpret_cast<Symbolic*>(
      cs_calloc(1, sizeof(Symbolic))); /* allocate symbolic analysis */
...
Numeric* cs_nfree(Numeric* N) {
  ...
  cs_free(N->Pinv);
  cs_free(N->B);
  return reinterpret_cast<Numeric*>(
...
Symbolic* cs_sfree(Symbolic* S) {
  ...
  cs_free(S->parent);
  cs_free(S->cp);
  return reinterpret_cast<Symbolic*>(
